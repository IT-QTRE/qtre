...
import { BlogPostDeleteAction } from "./blog-post-delete-action";

const STATUS_LABEL: Record<Doc<"blogPosts">["publishing"]["status"], string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};
...
const COLUMNS: ColumnDef<StockFeatures, Doc<"blogPosts">>[] = [
...
function titleMatches(post: Doc<"blogPosts">, query: string) {
  const needle = query.toLowerCase();
  return [post.title.en, post.title.ar, post.title.tr, post.publishing.slug]
    .filter(Boolean)
    .some((value) => value!.toLowerCase().includes(needle));
}

function BlogPostRowControls({ post }: { post: Doc<"blogPosts"> }) {
  const { menuOpen, setMenuOpen, confirmOpen, setConfirmOpen, requestDelete, onMenuOpenChangeComplete } = useRowMenuDelete();
  const setPublishingStatus = useMutation(api.blogPosts.setPublishingStatus);
  ...
  function applyStatus(next: "draft" | "published") {
    startTransition(async () => {
      try {
        await setPublishingStatus({ id: post._id, status: next });
        toast.success(next === "published" ? "Published" : "Moved to draft");
        setUnpublishOpen(false);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Failed to update post. Try again.");
      }
    });
  }
    ...
    <div className="flex shrink-0 items-center">
      ...
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} onOpenChangeComplete={onMenuOpenChangeComplete}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link href={`/admin/blog/${post._id}`} />}>Edit</DropdownMenuItem>
          {status === "archived" ? (
            <>
      ...
      <BlogPostDeleteAction
        blogPostId={post._id}
        blogPostTitle={post.title.en}
        open={confirmOpen}
...
export function BlogPostList({
  ...
  posts: Doc<"blogPosts">[];
  thumbnailByPostId: Map<string, PrimaryMediaItem>;
  toolbar?: ReactNode;
}) {
          ...
          {rows.map((row) => {
            ...
            return (
              <li key={post._id} className="flex items-center gap-2">
                ...
                >
                  <ThumbnailCell item={thumbnailByPostId.get(post._id)} alt={post.title.en} />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-heading text-sm font-medium">{post.title.en}</span>
                    <span className="mt-0.5 block truncate font-mono text-xs text-muted-foreground">
                      /en/blog/{post.publishing.slug}
                    </span>
                  </span>
                </Link>
                <BlogPostRowControls post={post} />
              </li>
            );
          })}
        </ul>
