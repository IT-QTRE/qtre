...
const STATUS_VARIANT: Record<Doc<"blogPosts">["publishing"]["status"], "outline" | "default" | "muted"> = {
  draft: "outline",
  published: "default",
  archived: "muted",
};
...
function BlogPostRowActions({ blogPost }: { blogPost: Doc<"blogPosts"> }) {
    ...
    <>
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} onOpenChangeComplete={onMenuOpenChangeComplete}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link href={`/admin/blog/${blogPost._id}`} />}>Edit</DropdownMenuItem>
      ...
      <BlogPostDeleteAction
        ...
        blogPostTitle={blogPost.title.en}
        open={confirmOpen}
...
// `filterColumnId="titleText"` (a flat string accessor) is what `DataTable`'s
// global filter actually searches against — same convention as project-columns.
export function getBlogPostColumns(
  thumbnailByBlogPostId: Map<string, PrimaryMediaItem>,
): ColumnDef<StockFeatures, Doc<"blogPosts">>[] {
  return [
    {
      id: "thumbnail",
      header: "",
      cell: ({ row }) => <ThumbnailCell item={thumbnailByBlogPostId.get(row.original._id)} />,
    },
    {
      id: "titleText",
    ...
    },
    {
      id: "actions",
      header: "",
      cell: ({ row }) => <BlogPostRowActions blogPost={row.original} />,
    },
  ];
}
