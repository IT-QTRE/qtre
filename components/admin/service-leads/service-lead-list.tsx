...
import { serviceDeskAdminLabel } from "@/lib/service-desks";
...
import { ServiceLeadDeleteAction } from "./service-lead-delete-action";

const STATUS_LABEL: Record<Doc<"serviceLeads">["status"], string> = {
  new: "New",
  contacted: "Contacted",
  qualified: "Qualified",
  closed: "Closed",
...
const COLUMNS: ColumnDef<StockFeatures, Doc<"serviceLeads">>[] = [
  { id: "nameText", accessorFn: (row) => row.name },
  { id: "status", accessorFn: (row) => row.status },
  { id: "createdAt", accessorFn: (row) => row.createdAt },
];

export function serviceLeadRegarding(lead: Doc<"serviceLeads">) {
  const group = lead.group === "visa" ? "Visa" : "License";
  return `${group} · ${serviceDeskAdminLabel(lead.group, lead.desk)}`;
}

function matchesSearch(
  lead: Doc<"serviceLeads">,
  query: string,
  ...
  const assignee = lead.assignedUserId
    ? (assigneeNameById.get(lead.assignedUserId) ?? (namesReady ? "Deleted admin" : undefined))
    : undefined;
  return [lead.name, lead.email, lead.phone, serviceLeadRegarding(lead), assignee]
    .filter(Boolean)
    .some((value) => value!.toLowerCase().includes(needle));
}

function ServiceLeadRowControls({ lead }: { lead: Doc<"serviceLeads"> }) {
  const { menuOpen, setMenuOpen, confirmOpen, setConfirmOpen, requestDelete, onMenuOpenChangeComplete } =
    useRowMenuDelete();
  return (
    <div className="flex shrink-0 items-center gap-1">
      ...
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} onOpenChangeComplete={onMenuOpenChangeComplete}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link href={`/admin/service-leads/${lead._id}`} />}>View</DropdownMenuItem>
          <DropdownMenuItem variant="destructive" onClick={requestDelete}>
            Delete
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      <ServiceLeadDeleteAction leadId={lead._id} leadName={lead.name} open={confirmOpen} onOpenChange={setConfirmOpen} />
    </div>
  );
}
...
function metaLine(
  ...
  const assignee = lead.assignedUserId
    ? (assigneeNameById.get(lead.assignedUserId) ?? (namesReady ? "Deleted admin" : undefined))
    : undefined;
  return [lead.email, serviceLeadRegarding(lead), assignee].filter(Boolean).join(" · ");
}

export function ServiceLeadList({
  leads,
  assigneeNameById,
  namesReady,
  toolbar,
}: {
  leads: Doc<"serviceLeads">[];
  assigneeNameById: Map<string, string>;
  namesReady: boolean;
  toolbar?: ReactNode;
}) {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, value) =>
      matchesSearch(row.original, String(value), assigneeNameById, namesReady),
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-sm">
          <label htmlFor="service-lead-search" className="sr-only">
            Search by name
          </label>
          <Input
            id="service-lead-search"
            name="service-lead-search"
            autoComplete="off"
      ...
      {rows.length === 0 ? (
        ...
        <ul className="divide-y divide-border border-y border-border">
          {rows.map((row) => {
            ...
            const line = metaLine(lead, assigneeNameById, namesReady);
            return (
              <li key={lead._id} className="flex items-center gap-2">
                <Link
                  href={`/admin/service-leads/${lead._id}`}
                ...
                <ServiceLeadRowControls lead={lead} />
              </li>
            );
          })}
        </ul>
