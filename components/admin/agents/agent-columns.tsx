...
const STATUS_VARIANT: Record<Doc<"agents">["publishing"]["status"], "outline" | "default" | "muted"> = {
  draft: "outline",
  published: "default",
  archived: "muted",
};
...
function AgentRowActions({ agent }: { agent: Doc<"agents"> }) {
...
export function getAgentColumns(thumbnailByAgentId: Map<string, PrimaryMediaItem>): ColumnDef<StockFeatures, Doc<"agents">>[] {
  return [
    {
      id: "thumbnail",
      header: "",
      cell: ({ row }) => <ThumbnailCell item={thumbnailByAgentId.get(row.original._id)} />,
    },
    {
      id: "nameText",
