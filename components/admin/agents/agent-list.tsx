...
import { AgentDeleteAction } from "./agent-delete-action";

const STATUS_LABEL: Record<Doc<"agents">["publishing"]["status"], string> = {
  draft: "Draft",
  published: "Published",
  archived: "Archived",
};
...
const COLUMNS: ColumnDef<StockFeatures, Doc<"agents">>[] = [
...
function nameMatches(agent: Doc<"agents">, query: string) {
  const needle = query.toLowerCase();
  return [agent.name, agent.position, agent.email, agent.phone]
    .filter(Boolean)
    .some((value) => value!.toLowerCase().includes(needle));
}

function AgentRowControls({ agent }: { agent: Doc<"agents"> }) {
  const { menuOpen, setMenuOpen, confirmOpen, setConfirmOpen, requestDelete, onMenuOpenChangeComplete } = useRowMenuDelete();
  const setPublishingStatus = useMutation(api.agents.setPublishingStatus);
  const [isPending, startTransition] = useTransition();
  const [unpublishOpen, setUnpublishOpen] = useState(false);
  const status = agent.publishing.status;
  ...
  function applyStatus(next: "draft" | "published") {
    startTransition(async () => {
      try {
        await setPublishingStatus({ id: agent._id, status: next });
        toast.success(next === "published" ? "Published" : "Moved to draft");
        setUnpublishOpen(false);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Failed to update agent. Try again.");
      }
    });
  }
    ...
    <div className="flex shrink-0 items-center">
      ...
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} onOpenChangeComplete={onMenuOpenChangeComplete}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link href={`/admin/agents/${agent._id}`} />}>Edit</DropdownMenuItem>
          {status === "archived" ? (
            <>
      ...
      <AgentDeleteAction agentId={agent._id} agentName={agent.name} open={confirmOpen} onOpenChange={setConfirmOpen} />
      <AlertDialog open={unpublishOpen} onOpenChange={setUnpublishOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hide this agent?</AlertDialogTitle>
            <AlertDialogDescription>
...
function metaLine(agent: Doc<"agents">) {
  return [agent.position, agent.email, agent.phone].filter(Boolean).join(" · ");
}
...
export function AgentList({
  ...
  agents: Doc<"agents">[];
  thumbnailByAgentId: Map<string, PrimaryMediaItem>;
  toolbar?: ReactNode;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        {filteredCount} {filteredCount === 1 ? "agent" : "agents"}
      </p>
      {rows.length === 0 ? (
          ...
          {rows.map((row) => {
            ...
            return (
              <li key={agent._id} className="flex items-center gap-2">
                ...
                >
                  <ThumbnailCell item={thumbnailByAgentId.get(agent._id)} alt={agent.name} objectTop />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-heading text-sm font-medium">{agent.name}</span>
                    {line ? <span className="mt-0.5 block truncate text-xs text-muted-foreground">{line}</span> : null}
                  </span>
                </Link>
                <AgentRowControls agent={agent} />
              </li>
            );
          })}
        </ul>
